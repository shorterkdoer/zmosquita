    ...
    <div class="col-<?= $f['col'] ?? 12 ?> <?= $bgClass ?>">
        ...
        <?php switch ($f['type']): 
            ...
            case 'decimal':
                ...
                $placeholder_val = $f['placeholder'] ?? '';
                $attr_placeholder = (!empty($placeholder_val)) ? 'placeholder="' . htmlspecialchars($placeholder_val) . '"' : '';
                $attr_step = ($f['step'] ?? false) ? "step='" . htmlspecialchars($f['step']) . "'" : '';

                ?>
                <input type="number" <?= $attr_step ?> class="form-control" name="<?= htmlspecialchars($f['nombre']) ?>" id="<?= htmlspecialchars($f['nombre']) ?>"
                    value="<?= htmlspecialchars($values[$f['nombre']] ?? '') ?>" <?= $attr_required ?> <?= $readonly ?> <?= $attr_placeholder ?> >
            ...
            <?php break;   
            case 'time': ?>
                <input type="time" class="form-control" name="<?= $f['nombre'] ?>" id="<?= $f['nombre'] ?>"  value="<?= $values[$f['nombre']] ?>">
            <?php break;  
            case 'date': 
...
<?php foreach ($buttons as $button): 
    ...
    <?php elseif ($button['backbutton'] ?? false): ?>
...
function buildSelect(string $name, array $options, $selected = null, bool $disabled = false): string 
{
    ...
    foreach ($options as $option) {
        ...
        $optText  = htmlspecialchars($option['label']);
        $isSelected = ($selected !== null && $selected == $option['id']) ? ' selected' : '';
        $html .= "<option value='{$optValue}'{$isSelected}>{$optText}</option>\n";
    }
    $html .= "</select>\n";
...
/*
    if ( is_null($options) || empty($options)) {
        $html .= "<option value=''>" . htmlspecialchars('No hay valores...') . "</option>\n";
    }else {
        foreach ($options as $option) {
            $optValue = htmlspecialchars($option['id']);
            $optText  = htmlspecialchars($option['nombre']);
            foreach ($option['suboptions'] as $suboption) {
                $subValue = htmlspecialchars($suboption['id']);
                $subText  = htmlspecialchars($suboption['nombre']);

                $html .= "<option value='{$subValue}'>{$subText}</option>\n";
        }
