...
use App\Models\Log_small;
use PDO;

class Log_smallController extends Controller
{
    public function mainview(): void
    {
        /* revisar los paths */
        $this->makegrid('cruds/log_small/index.php');
    ...
    $cfgedit     = require $_SESSION['directoriobase'] . '/config/cruds/log_small/index.php';
    $cfg         = $cfgedit['config']    ?? [];
    ...
    $datos = Log_small::CustomQry($query);
    ...
    public function create(): void
    {
        ...
        if (!$id) {
            ...
            $this->redirect('/log_small'); // Redirect to the main view if no ID is provided
            return;
        }
        $crudstyle = require $_SESSION['directoriobase'] . '/config/cruds/defaults/crudstyle.php';
        $style = $crudstyle['style'] ?? [];
        $cfgcreate = require $_SESSION['directoriobase'] . '/config/cruds/log_small/create.php';
    ...
    public function edit($request, array $params): void
    {
        ...
        $cfgedit     = require $_SESSION['directoriobase'] . '/config/cruds/log_small/edit.php';
        $cfg         = $cfgedit['config']    ?? [];
        $cfg['url_action'] .= '/' . $id; // <— se agrega el id a la url
        $campos      = $cfgedit['campos']    ?? [];
        $actividades = $cfgedit['actividades'] ?? [];
    ...
    public function update($request, array $params): void
    {
        ...
        if (!Log_small::find($id)) {
            Session::flash('error', 'Comprobante no encontrado.');
            $this->redirect('/miscomprobantes');
            return;
        }

        Log_small::update($id, ['nombre' => $nombre]);
